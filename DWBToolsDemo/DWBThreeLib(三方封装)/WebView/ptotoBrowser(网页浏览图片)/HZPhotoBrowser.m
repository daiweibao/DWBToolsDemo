...
- (UILongPressGestureRecognizer *)longPressGest {
    if (!_longPressGest) {
        _longPressGest = [[UILongPressGestureRecognizer alloc]initWithTarget:self action:@selector(longPressView:)];
    }
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo;
{
    if (error) {
        ...
        [DWBToast showCenterWithText:HZPhotoBrowserSaveImageFailText];
    } else {
         [DWBToast showCenterWithText:HZPhotoBrowserSaveImageSuccessText];
    }
}
...
- (void)setPhotoView {
    ...
    self.photoView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    self.photoView.backgroundColor = [UIColor colorWithWhite:0.2 alpha:0.4];
    [self addSubview:self.photoView];
    
    
    self.whiteView = [[UIView alloc] init];
    if (iPhoneX) {
        self.whiteView.frame = CGRectMake(0, SCREEN_HEIGHT, SCREEN_WIDTH, 134);
        
    }else {
        self.whiteView.frame = CGRectMake(0, SCREEN_HEIGHT, SCREEN_WIDTH, 100);
        
    }
    self.whiteView.backgroundColor = [UIColor whiteColor];
    [self.photoView addSubview:self.whiteView];
    
    UIButton *photographButton = [UIButton buttonWithType:UIButtonTypeCustom];
    photographButton.frame = CGRectMake(0, 0, SCREEN_WIDTH, 49);
    [photographButton setTitle:@"保存" forState:UIControlStateNormal];
    [photographButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [photographButton addTarget:self action:@selector(saveBtnClick) forControlEvents:UIControlEventTouchUpInside];
    [self.whiteView addSubview:photographButton];
    
    UIImageView *lineImg = [[UIImageView alloc]init];
    lineImg.frame = CGRectMake(0,49,SCREEN_WIDTH, 1);
    lineImg.backgroundColor = [UIColor groupTableViewBackgroundColor];
    [self.whiteView addSubview:lineImg];
    UIButton *albumButton = [UIButton buttonWithType:UIButtonTypeCustom];
    albumButton.frame = CGRectMake(0, 50, SCREEN_WIDTH, 50);
    [albumButton setTitle:@"取消" forState:UIControlStateNormal];
    ...
    [self.whiteView addSubview:albumButton];
    
    UITapGestureRecognizer *singleRecognizer = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(SingleTap:)];
    //点击的次数
    singleRecognizer.numberOfTapsRequired = 1; // 单击
    //给self.view添加一个手势监测；
    [self.photoView addGestureRecognizer:singleRecognizer];
}
- (void)saveBtnClick {
    [self saveImage];
    [self setHiddenPhotoView];
...
- (void)setShowPhotoView {
    [UIView animateWithDuration:0.3 animations:^{
        self.photoView.alpha = 1;
        if (iPhoneX) {
            self.whiteView.frame = CGRectMake(0, SCREEN_HEIGHT-134, SCREEN_WIDTH, 134);
            
        }else {
            self.whiteView.frame = CGRectMake(0, SCREEN_HEIGHT-100, SCREEN_WIDTH, 100);
            
        }
    }];
    
}
- (void)setHiddenPhotoView {
