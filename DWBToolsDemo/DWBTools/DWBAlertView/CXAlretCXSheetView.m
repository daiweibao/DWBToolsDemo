...
#import "CXAlretCXSheetView.h"

@interface CXAlretCXSheetView()
//外部传入的值
...
@implementation CXAlretCXSheetView
...
+ (void)AlertMyCXSheetViewWithController:(UIViewController*)controller Title:(NSString*)title otherItemArrays:(NSArray *)array ShowRedindex:(NSInteger )redIndex isShowCancel:(BOOL)isShow CancelTitle:(NSString*)cancetitle Type:(NSInteger)type handler:(ActionBlockAtIndex)block{
    ...
    //控件不存在才创建，防止重复创建
    if (viewWX==nil) {
        CXAlretCXSheetView * alertView = [[CXAlretCXSheetView alloc]init];
...
- (void)setUpContentViewAray:(NSArray*)array{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        contentView.backgroundColor = UIColorFromRGB(0xF1F1F1);
        //添加点击手势拦截
...
-(void)createUITwo:(NSArray*)array{
    ...
    for (int i = 0; i < array.count; i++) {
        ...
        if (i == self.redIndex) {
            ...
            [buttonOther setTitleColor:UIColorFromRGB(0x222222) forState:UIControlStateNormal];
        ...
        imageLine.backgroundColor = UIColorFromRGB(0xF1F1F1);
    ...
    if (array.count == self.redIndex) {
        ...
        [buttonCancel setTitleColor:UIColorFromRGB(0x222222) forState:UIControlStateNormal];
    }
